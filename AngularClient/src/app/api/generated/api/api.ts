export * from './accountingDataset.service';
import { AccountingDatasetService } from './accountingDataset.service';
export * from './balances.service';
import { BalancesService } from './balances.service';
export * from './caseList.service';
import { CaseListService } from './caseList.service';
export * from './currencies.service';
import { CurrenciesService } from './currencies.service';
export * from './currencyExchange.service';
import { CurrencyExchangeService } from './currencyExchange.service';
export * from './dataset.service';
import { DatasetService } from './dataset.service';
export * from './documentCategory.service';
import { DocumentCategoryService } from './documentCategory.service';
export * from './documents.service';
import { DocumentsService } from './documents.service';
export * from './electricity.service';
import { ElectricityService } from './electricity.service';
export * from './file.service';
import { FileService } from './file.service';
export * from './gas.service';
import { GasService } from './gas.service';
export * from './incoming.service';
import { IncomingService } from './incoming.service';
export * from './mBankScrapper.service';
import { MBankScrapperService } from './mBankScrapper.service';
export * from './mazda.service';
import { MazdaService } from './mazda.service';
export * from './santanderScrapper.service';
import { SantanderScrapperService } from './santanderScrapper.service';
export * from './settlement.service';
import { SettlementService } from './settlement.service';
export * from './skoda.service';
import { SkodaService } from './skoda.service';
export * from './statistics.service';
import { StatisticsService } from './statistics.service';
export * from './transactions.service';
import { TransactionsService } from './transactions.service';
export * from './tutoring.service';
import { TutoringService } from './tutoring.service';
export * from './tutoringList.service';
import { TutoringListService } from './tutoringList.service';
export * from './water.service';
import { WaterService } from './water.service';
export const APIS = [AccountingDatasetService, BalancesService, CaseListService, CurrenciesService, CurrencyExchangeService, DatasetService, DocumentCategoryService, DocumentsService, ElectricityService, FileService, GasService, IncomingService, MBankScrapperService, MazdaService, SantanderScrapperService, SettlementService, SkodaService, StatisticsService, TransactionsService, TutoringService, TutoringListService, WaterService];
